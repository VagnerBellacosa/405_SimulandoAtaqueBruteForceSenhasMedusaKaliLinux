...
IntroduçãoPrincipais Tipos de AtaquesPassword Spraying e Credential StuffingHydra no Kali LinuxNcrack no Kali LinuxJohn no Kali LinuxWPScan no Kali LinuxPatator no Kali LinuxAmbiente com Kali Linux e Metasploitable 2Alcançando a Máquina Vulnerável no Metasploitable 2Criando Arquivos de Usuários e Senhas ComunsAtaques de Força Bruta em Formulários de Login WebCriação de WordlistsUtilizando o Medusa para Simular Combinações de AcessoAtaque em Cadeia: Enumeração SMB e Password SprayingSimulando um Cenário Corporativo Mal ConfiguradoCriando uma Lista de UsuáriosTestando o Acesso Utilizando SmbclientEncerramentoAula Extra: Limitações do Medusa e Dicas FinaisEntendendo o Desafio